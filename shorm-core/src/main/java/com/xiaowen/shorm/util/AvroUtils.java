...
public class AvroUtils {
     ...
     *
     * @param schema the persistent object to get the fields names from
     * @return the field names
     */
    ...
    public static <T extends Persistent> T deepClonePersistent(T persistent) {
        ...
        try {
            ...
            throw new RuntimeException("Unable to serialize avro object to byte buffer.");
        }
        byte[] value = bos.toByteArray();
        Decoder dec = DecoderFactory.get().binaryDecoder(value, null);
        @SuppressWarnings("unchecked")
        SpecificDatumReader<T> reader = new SpecificDatumReader<>((Class<T>) persistent.getClass());
        try {
            return reader.read(null, dec);
        } catch (IOException e) {
            throw new RuntimeException("Unable to deserialize avro object from byte buffer.");
        }
    }
}
