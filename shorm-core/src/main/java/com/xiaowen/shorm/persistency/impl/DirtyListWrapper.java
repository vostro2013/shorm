 ...
 *
 * @param <T> The type of the list that this wrapper wraps.
 *
 * @author: wenc.hao
...
public class DirtyListWrapper<T> extends DirtyCollectionWrapper<T> implements Dirtyable, List<T> {
     ...
     *
     * @param delegate The getDelegate().to wrap.
     */
    ...
    public ListIterator<T> listIterator(int index) {
        return new DirtyListIterator<>(getDelegate().listIterator(index), getDirtyFlag());
    }

    @Override
    public List<T> subList(int fromIndex, int toIndex) {
        return new DirtyListWrapper<>(getDelegate().subList(fromIndex, toIndex), getDirtyFlag());
    }
